...
def validate_snapshot(source):
    try:
        ...
        if manifest['format']!='openiq-backup-v1':raise ValueError()
        ...
        if filename is None or set(manifest['sha256'])!={filename,'.secret-key'}:raise ValueError()
        for name,digest in manifest['sha256'].items():
            path=source/name
            if path.is_symlink() or not path.is_file():raise ValueError()
            with path.open('rb') as stream:
                if hashlib.file_digest(stream,'sha256').hexdigest()!=digest:raise ValueError()

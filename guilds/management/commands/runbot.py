...
class Command(BaseCommand):
    ...
    def add_arguments(self,parser):parser.add_argument('--sync',action='store_true');parser.add_argument('--check',action='store_true')
    def handle(self,*args,**options):
        intents=discord.Intents.default()
        class Bot(discord.Client):
            def __init__(self):super().__init__(intents=intents);self.tree=app_commands.CommandTree(self)
            async def setup_hook(self):
                if options['sync']:await self.tree.sync()
        bot=Bot();groups={}

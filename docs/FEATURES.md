...
| War history | Create/edit/delete, dates/types/results/caps, participant add/remove via form, class snapshot, K/D exclusion and alliance sharing, notes, workflow status, direct signup-event/live-session linking | Exact original accolade formulas and visual styling; sortable detail tables, inline scores/classes/exclusions, row removal and top-class/award summaries are implemented |
